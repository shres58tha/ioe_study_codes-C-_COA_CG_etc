...
//Method to register node numbers as char type variable
...
//Define scope and limitation of graph
class Graph
{
public:
    Graph();
...
private:
    ...
    vector<char> nodeName;	                                 // Map node numbers into node names
    map<char, int> nodeNum;	                                // Map node names into node numbers
...
Graph::Graph(int numVertices, int initialValue=INFINIT)
{
    ...
    numV = numVertices;
    numE = 0;
    nodeName.resize(numVertices);
    for (int x=0; x<numV; ++x)
    {  
        nodeName[x] = convert(x);
        nodeNum[convert(x)]=x;
    }
    adjList.clear();
...
//Return node name registered to respective node number
...
{
    int posX = nodeNum[x];	                                   // Get the number of node 'x'
    nodeName[posX] = name;	                                   // Link node number to 'name'
    nodeNum[name]=posX;	                                      // Link 'name' to node number
}
...
void Graph::set_edge_value(char x, char y, int value)
{
    ...
    for(list<Node>::iterator i=adjList.begin(); i != adjList.end(); ++i)
    {
        if ((*i).number==nodeNum[y])
        {
            found = false;
            for(list<Node>::iterator j=(*i).edges.begin(); j != (*i).edges.end(); ++j)
            {
                if ((*j).number==nodeNum[x])
                {  
                    (*j).weight=value;
                    found = true;
                }
...
// Boolean argument for neighboring and non-neighboring nodes
...
// Return a list containing the list of neighbors of 'x'
list<char> Graph::neighbors(char x)
{
    ...
    for(list<Node>::iterator i=adjList.begin(); i != adjList.end(); ++i)
    {
        if ((*i).number==nodeNum[x])
        {
            for(list<Node>::iterator j=(*i).edges.begin(); j != (*i).edges.end(); ++j)
            {
                adjNodes.push_back(nodeName[(*j).number]);
            }
        }
    }
    return adjNodes;
...
list<char> Graph::vertices()
{
    ...
    for(list<Node>::iterator i=adjList.begin(); i != adjList.end(); ++i)
    {
        nodes.push_back(nodeName[(*i).number]);
    }
    return nodes;
}
...
struct strNodeInfo
{
    ...
    int Disp;	                                                // Shortest displaced path found to nodeName
...
// Compare NodeInfo by Disp
bool compareDisp(NodeInfo& n1, NodeInfo& n2)
{
    if (n1.Disp < n2.Disp) return true;
    return false;
}
...
private:
    list<NodeInfo> pq;		                                    // List of known nodes/paths ordered by Disp
};
...
// Remove the node with lower Disp from priority queue 
...
// Return true if node 'n' has a lower Disp than the node with the same name in the priority queue and false otherwise
...
// Return the node with lower Disp in priority queue (without removing it from the queue))
...
list<char> ShortestPath::path(char u, char w)
{
    ...
    while ((!candidates.empty()) && (lastSelected.nodeName !=w))
    {
        ...
        {
            n.nodeName=*i;
            n.Disp=lastSelected.Disp+graph.get_edge_value(lastSelected.nodeName,*i);
            n.through=lastSelected.nodeName;
            if (!p.contains(n))	                                  // Add candidate to priority queue if doesn't exist 
            p.insert(n);
            else
            if (p.isBetter(n))	                                        // Update candidate Disp in priority queue if a better path was found
            p.chgPriority(n);
        }
        lastSelected = p.top();			                              // Select the candidate with Disp from priority queue
...
public:
    MonteCarlo();
    Graph randomGraph(int vert, double density, int DispEdge, int maxDistEdge);
    void run(Graph g);
...
Graph MonteCarlo::randomGraph(int numVert, double density, int DispEdge, int maxDistEdge)
{
    int randDistEdge;
    ...
    for (int j=i+1; j<g.V(); ++j)
    {
        ...
        if (p < density)	                                     // If random probability is less than density, edge (i,j) will be set
        {
            ...
            randDistEdge = rand() % (maxDistEdge - DispEdge) + DispEdge;
            srcVert = convert(i);
            dstVert = convert(j);
...
void MonteCarlo::run(Graph g)
{
    ...
    // Calculate real density reached
    double d = static_cast<double>(g.E())/((static_cast<double>(g.V())*static_cast<double>(g.V())-1)/2)*100;
