 ...
 * Answer*/
/*
Operator overloading is a concept in C++ which provide the new definition for the operator that are defined by the language. It is a type of static polymorphism in which an operator is overloaded to give user defined meaning to it, performing operation on user-defined data type. For example '+' operator can be overloaded to perform addition on various data types, objects type, and even to add   Strings (i.e. concatenation) etc.
...
polar polar::operator + (polar point){ 
    return polar( rect(*this) + rect(point) );  //  type conversion twice
}
