; 8086 code to read a string apd display each word in a separate line in the center of the screen.
      .model small
      .stack 100h
       ...
       .code
        main proc 
            ...
            setwindow macro
                ...
                mov bh,13h          ; black background and white foreground 
                mov cx,00h        ; ch=00h clear screen bring cursor to 00 00 
                mov dh,row          ; set row
                mov dl,col          ; set col
            ...
            disp_center macro msg 
                ...
                mov dh,12           ; row/2
                mov dl,40           ; col/2
                mov bl,15           ; ?? max size of the word
                mov si,offset msg   ; get address of msg in si
     nxtdis:    mov ah,02h          ; set cursor
                ...
                dec bl              ; ??
                mov bh,[si]         ; get next char
                cmp bh,' '          ; compare with white space
                jnz nxtdis          ; loop
                inc dh              ; point to next row
                mov dl,40          ; center of the coulums  col/2
                dec bl              ;?? 
                jnz nxtdis          ; probably max 15 words space..  as a safe guard for infinite loop
            endm
