...
#include <time.h>
#include <stdlib.h>
// use file to save reesults
// file structure using fgets and fputs
// linked list of matrix
// or create a buffer of sizeof(matrix*no of matrix required +10) 
// use sscanf and sprintf or fgets fread in memory
// finally write that to the file
...
typedef struct matrix_{
    int row;
    int col;
    float **mat;            // 2D arrary rep using poing
}matrix;
matrix nullmatrix={0,0,NULL}; // defining initiazation of null matrix
...
int main(){
         ...
         initialize(&b,2); // takes adress of matrix type and integer 0 for initialization with 0, 1= rand, 2=user input
         display_mat(&b);
         // add sub
