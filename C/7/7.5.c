...
int str_ch_all_loc(const char *s, char ch, int *p){  // puts all the position of char ch starting from count 1 in arr p and returns j no of ch repeats
        int  i=0,j=0;
...
void strcp(const char *s, char *t){
        ...
        //while (*s){*t=*s;s++;t++;}  *t='/0';
        //while (*s){*t++=*s++;}  *t='/0';  //untested but should work
        }
