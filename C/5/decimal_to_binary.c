...
// update nice logic but found error, actually it is converting it to the binary encoded decimal not binary. and also to binary using hex arguments
...
int count_hex_digits(long long int num){
    int num_digit=0;
    while (num){ num=num/16; num_digit++; }
    return num_digit;
    }   // return number of digits in given number

int count_base_digits(long long int num, int base){  // count digits in base
    int num_digit=0;
...
//long long bin=0;
    ...
    num_digit=count_hex_digits(num); // num_digits becomes no of digits in the number
