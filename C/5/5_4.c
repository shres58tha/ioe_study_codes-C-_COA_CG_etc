// program to print binary equivalent of an entered number
#include<stdio.h>
#include <stdlib.h>
//#include<math.h>

#define block 3
                    //block 2 is refined code
...
return (ret_val); // needs to nbe last line in the function.
 ...
 long long int deci_bin( int num){
     long long int  bin_num=0, place=1;
        ...
        while (num){
        bin_num=bin_num+num%2*place;  // extract least significant bit equivalent and set it on least significant bit in bin_num
...
long long int deci_bin( int num){
     long long int  bin_num=0, place=1;
        if(num >524287) {printf("mem overflow max decimal no = 524287  \n"); exit (1);}
        while (num){
        bin_num = bin_num + num%2*place;  // extract least significant bit equivalent and set it on least singificant bit in bin_num
        num/=2;  // setnum to extract next least significant bit
        place*=10;
        // printf("%lld\n",bin_num);  // debug line
        }
