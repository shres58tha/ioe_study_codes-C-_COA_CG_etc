...
{ 
    int len = strlen(num);                                      // Get number of digit in given number 
    ...
    char *digit_1[] = {"zero","one","two","three","four","five","six","seven","eight","nine"}; 
    ...
    while (*num != '\0') { 
        ...
        /* Code path for first 2 digit */
        if (len >= 3) { 
            if (*num -'0' != 0) { 
                printf("%s ", digit_1[*num - '0']); 
                printf("%s ", tens_power[len-3]);                           // here len can be 3 or 4 
              } 
            --len; 
          } 

        /* Code path for last 2 digit */
        else { 
            /* Need to explicitly handle 10-19. Sum of the two digit is 
            used as index of "two_digit" array of strings */
            if (*num == '1') { 
